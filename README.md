...
Each Agents API session has a compact, one-hour many-to-many participant index.
Redis stores both the users participating in a session and the sessions in
which a user participates. A reply to a message handled by Alfred (including
Alfred's own reply) continues that exact session and adds the replying user as
a participant. A non-reply invocation continues the sender's most recently
active session, or starts a new one when none exists.
