...
class ChatGPTClient:
    ...
    def prepare_weather_report(self, raw_report: dict) -> str:
        ...
        prompt = f"""Проанализируй JSON с ежедневными данными для Еревана.
        Поле air_quality содержит ответ IQAir и используется только для качества воздуха.
        Поле weather содержит ответ Яндекс.Погоды и является единственным источником
        текущей погоды и прогноза. Составь отчёт о качестве воздуха и прогнозе погоды.
